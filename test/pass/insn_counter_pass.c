...
static void add_counter_to_bb(struct bpf_ir_env *env, struct ir_basic_block *bb,
                              struct ir_function *fun, struct bpf_ir_counter_opt opt,
    ...
    if (!last || !bpf_ir_is_jmp(last)) {
        ...
        struct ir_insn *added = bpf_ir_create_bin_insn(env, load_insn, bpf_ir_value_insn(load_insn),
                                                       cv, IR_INSN_ADD, IR_ALU_64, INSERT_BACK);
        struct ir_insn *store_back =
            bpf_ir_create_store_insn(env, added, alloc_insn, bpf_ir_value_insn(added), INSERT_BACK);
        ...
        bpf_ir_create_jbin_insn(env, store_back, bpf_ir_value_insn(added),
                                bpf_ir_value_const32(opt.counter_limit), *succ, err_bb, IR_INSN_JGT,
                                IR_ALU_64, INSERT_BACK);
        bpf_ir_connect_bb(env, bb, err_bb);
        return;
    }
    struct ir_insn *load_insn =
        bpf_ir_create_load_insn(env, last, bpf_ir_value_insn(alloc_insn), INSERT_FRONT);
    struct ir_value cv    = bpf_ir_value_const32(-1);
    cv.builtin_const      = IR_BUILTIN_BB_INSN_CRITICAL_CNT;
    struct ir_insn *added = bpf_ir_create_bin_insn(env, load_insn, bpf_ir_value_insn(load_insn), cv,
    ...
    // bpf_ir_create_throw_insn_bb(env, err_bb, INSERT_BACK);
    bpf_ir_create_jbin_insn(env, store_back, bpf_ir_value_insn(added),
                            bpf_ir_value_const32(opt.counter_limit), new_bb, err_bb, IR_INSN_JGT,
                            IR_ALU_64, INSERT_BACK);
...
void insn_counter(struct bpf_ir_env *env, struct ir_function *fun, void *param) {
    ...
    struct ir_insn        *alloc_insn =
        bpf_ir_create_alloc_insn_bb(env, entry, IR_VR_TYPE_32, INSERT_FRONT);
