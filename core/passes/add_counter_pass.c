		...
		array_for(pos, critical_bbs)
		{
			...
			if (!last || !bpf_ir_is_jmp(last)) {
				...
				struct ir_insn *load_insn =
					bpf_ir_create_load_insn_bb(
						env, bb,
						bpf_ir_value_insn(alloc_insn),
						INSERT_BACK);
				...
				struct ir_insn *added = bpf_ir_create_bin_insn(
					env, load_insn,
					bpf_ir_value_insn(load_insn), cv,
				...
				bpf_ir_create_throw_insn_bb(env, err_bb,
							    INSERT_BACK);
			...
			struct ir_insn *load_insn = bpf_ir_create_load_insn(
				env, last, bpf_ir_value_insn(alloc_insn),
				INSERT_FRONT);
			struct ir_value cv = bpf_ir_value_const32(-1);
			...
			struct ir_basic_block *new_bb = bpf_ir_split_bb(
				env, fun, store_back, INSERT_BACK);
			struct ir_basic_block *err_bb =
				bpf_ir_create_bb(env, fun);
